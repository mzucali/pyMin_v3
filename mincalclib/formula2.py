...
def formula_from_oxides(mineral_Ox_dict):
    ...
    print(".dict_oxygens_prop type ", type(oxygens_prop_dict))
    ...
    dict_unsorted_data_recalc_Ox = compareLabelsDictDataToListOx(dict_data, list_oxides_order)
    dict_ordered_Ox = sortDictOxides(dict_unsorted_data_recalc_Ox)
    ...
    for k in list_oxides_order: # controlla tutti i termini nella list_oxides_order (e.g., sample, mineral
        ...
        print("list_oxides_order: ",k)
        if k.lower() in list(data_recalc_Ox_dict.keys()): # se quel valore è in
    ...
    data_Ox_with_OX_list.append(dict_ordered_Ox)
